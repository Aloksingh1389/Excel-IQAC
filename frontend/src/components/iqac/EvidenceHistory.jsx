...
import { History, MessageSquare, Tag } from 'lucide-react';
import { EvidenceStatusBadge } from './EvidenceStatusBadge';

export const EvidenceHistory = ({ history = [] }) => {
  return (
      ...
      <div className="space-y-4 relative pl-4 border-l-2 border-slate-200">
        {history.length === 0 ? (
          <p className="text-xs text-slate-400 text-center py-4">
            No evidence version history logged.
          </p>
        ) : (
          history.map((item) => (
            <div key={item.id} className="relative space-y-1.5">
              ...
              <div className="flex items-center justify-between text-xs flex-wrap gap-2">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded bg-indigo-100 text-indigo-900 font-black text-[10px]">
                    v{item.version || 1}
                  </span>
              ...
              {item.previousStatus && item.newStatus && (
                <div className="flex items-center gap-1.5 text-[10px] font-medium text-slate-500">
                  <span>Status:</span>
                  <EvidenceStatusBadge status={item.previousStatus} />
                  <span>&rarr;</span>
                  <EvidenceStatusBadge status={item.newStatus} />
                </div>
              )}
