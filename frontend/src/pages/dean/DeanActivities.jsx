...
export const DeanActivities = () => {
  ...
  useEffect(() => {
    ...
    setError(null);
    Promise.all([
      deanPortalService.getDeanActivities({
  ...
  return (
    <div className="space-y-6">
      <div className="pb-4 border-b border-slate-200/80">
        ...
        <p className="text-xs text-slate-500 font-medium mt-0.5">Department activities across your assigned scope (monitoring only)</p>
      </div>
      <Card className="p-4 sm:p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          ...
          <select value={statusFilter} onChange={(e) => set('status', e.target.value)} className={inputCls}>
            {['ALL', 'PLANNED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'OVERDUE'].map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
      </Card>
      <Card className="p-4 sm:p-6 space-y-3">
        <h3 className="text-sm font-bold text-slate-900">Activities ({(items || []).length})</h3>
        {(items || []).length === 0 ? <EmptyState title="No activities" description="No activities match the current filters." /> : (
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {(items || []).map((a) => (
                  <tr key={a.id} className="hover:bg-slate-50/80">
                    ...
                    <td className="p-3 text-right"><Link to={`/dean/activities/${a.id}`} className="px-2.5 py-1 rounded-lg bg-indigo-600 text-white font-bold">View</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
